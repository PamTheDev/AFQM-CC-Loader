...
ScriptMessage($"{gmlFileName} created and imported.");
...
portraitBuilder.Append("validCharacters = [\"eureka\", \"knockt\", \"rend\"");

// Insert JSON contents, separated by commas, as additional array elements (no trailing or double commas)
bool addedAny = false;
for (int i = 0; i < portraitJsonFiles.Length; i++)
{
    string jsonContent = File.ReadAllText(portraitJsonFiles[i]).Trim();
    if (!string.IsNullOrWhiteSpace(jsonContent))
    {
        portraitBuilder.Append(", ");
        portraitBuilder.Append(jsonContent);
        addedAny = true;
    }
}
...
ScriptMessage($"{portraitFileName} created and imported.");
